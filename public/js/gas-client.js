(function(){
  ...
  async function gasFetch(params={}, method='GET', timeoutMs=12000){
    ...
    try{
      ...
      // Fallback JSONP (สำหรับกรณี CORS)
  ...
  async function loadDashboard(){
    try{
      // ตัวอย่างเรียกข้อมูลรวม (action=dashboard หรือ health)
      const data = await gasFetch({ action: 'dashboard' }, 'GET');
      // วางข้อมูลลง DOM ง่ายๆ (สหายปรับต่อกับตารางเดิมได้)
      const el = document.querySelector('[data-km="dashboard-json"]');
      if (el) el.textContent = JSON.stringify(data, null, 2);
      // TODO: map data.sheets / data.tasks ฯลฯ ไปยังตารางจริงของหน้าแดชบอร์ด
    }catch(e){
      console.error(e);
      const el = document.querySelector('[data-km="dashboard-json"]');
