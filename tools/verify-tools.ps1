...
if (-not $repoRoot) { Add-Fail "Not inside a git repository."; goto :SUMMARY }
...
foreach($f in $required){ if(Test-Path $f){ Add-Ok "$f found" } else { Add-Fail "$f missing" } }

# 2) git-wip functions / aliases (ทน policy)
try {
...
if (Test-Path $gaPath) {
  ...
  if ($ga -notmatch "(?im)\*\.mp3\s+.*filter=lfs") { Add-Ok ".gitattributes: no LFS rule for *.mp3" } else { Add-Warn ".gitattributes: LFS rule for *.mp3 still present" }
